...
class DelObject(APIView):
    def delete(self, request):
        ...
        score_table = Score.objects.get(anime_id=anime_id, user_id=user_id)
        score_table.delete()
        return Response({'message': 'Объект успешно удален из списка'}, status=status.HTTP_200_NO_CONTENT)
...
class DelMangaObject(APIView):
    def delete(self, request):
        ...
        score_table = Score_manga.objects.get(manga_id=manga_id, user_id=user_id)
        score_table.delete()
        return Response({'message': 'Объект успешно удален из списка'}, status=status.HTTP_204_NO_CONTENT)     
